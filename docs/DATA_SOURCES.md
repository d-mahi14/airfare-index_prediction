...
- **Flight Number Disambiguation:** Because the dataset only provides coarse time bands (e.g. "After 6 PM") rather than exact departure times, distinct flights operating the same route on the same day in the same class may share the exact same `Flight_code` and time band. To prevent data loss and allow these to exist simultaneously in the database without violating unique constraints, the data loader will automatically append a synthetic sequence suffix to the flight number (e.g., `6E-107-2`) for any subsequent collisions. These suffixes are disambiguation artifacts and not part of the real airline flight code.
